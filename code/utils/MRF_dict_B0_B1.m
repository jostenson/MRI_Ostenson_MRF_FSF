function output = MRF_dict_B0_B1(input)
...
dict_list = zeros(nT2*nT1*nB1*nB0,4);
...
parfor ii = 1:dict_length
    ...
    disp(['MRF dictionary construction: T1 ',num2str(T1),' :: T2 ',...
        num2str(T2), ' :: B1 ', num2str(B1), ' :: B0 ', num2str(B0)]);
    
    % run sequence using EPG
    tmp_v = EPG_MRF_SSFP_B0( T1,T2,TE_v,TR_v,myFA_v,delk,nreps,szomega,phi_v,TI,B0,i_sign );
