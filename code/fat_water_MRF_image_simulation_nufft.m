%% create synthetic unbalanced SSFP fixed TR MRF image
...
for ii = 1:n_segs
    ...
    for jj = 1:my_n_B1
        ...
        my_signal_v = EPG_MRF_SSFP( T1_water_ms_v(ii), T2_water_ms_v(ii), TE_ms_v, TR_ms_v, my_B1*FA_deg_v, delk, n_TR, szomega, phi_v, TI_ms );
...
for ii = 1:n_B1_unique
    ...
    if any( my_B1 == B1_true_fat(:) )
        ...
        for jj = 1:n_fat_pks
            ...
            my_signal_v = EPG_MRF_SSFP( T1_ms, T2_ms, TE_ms_v, TR_ms_v, my_B1*FA_deg_v, delk, n_TR, szomega, phi_v, TI_ms );
        ...
        for jj = 1:N
            for kk = 1:N
                ...
                for ll = 1:n_fat_pks
                    ...
                    phasor_v = exp( i_sign * 1i * 2 * pi * f_Hz * ( acq_time_s + TE_ms_v(:)/1000 ) );
...
if B0_pert == 1     
    ...
    for ii = 1:n_B0
        ...
        for jj = 1:n_TR
            ...
            my_t_map_s = TE_ms_v(jj)/1000 + acq_time_map_s;
